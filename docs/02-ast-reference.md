...
**`Set` literal vs `set()` constructor:** The `Set` node represents Python's literal set syntax `{1, 2, 3}` — this is unsupported. However, the `set()` **constructor** (e.g., `set([1, 2, 3])`) is a `Call` to `Name("set")`, which IS supported via the builtins table (§12.8) and maps to `MapSet.new(Enum.to_list(x))`. Similarly, set mutation methods like `set.add()`, `set.discard()`, etc. are supported via the mutation methods table (§9.4). The distinction: the AST node `Set` represents literal syntax with no equivalent, while the builtin function `set()` is a constructor call that maps cleanly to `MapSet`.
