...
- `lib/pylixir/context.ex`: struct per §10.2 (`scopes`, `while_counter`, `loop_nesting`, `known_functions`, `temp_counter`, `module_attrs`, `def_position`). `temp_counter` is the shared monotonic counter for single-evaluation temps generated in T12 (chained `Compare` middles), T13 (multi-target assign RHS), and T14 (`AugAssign` subscript value/slice). `module_attrs` is the `MapSet` of Python names that became `@var_<name>` module attributes (populated by T05's Pass 1). `def_position` is `:module_top | :nested_fn | :other` — drives T19's reject-FunctionDef-inside-control-flow check.
...
- **Module-body partitioning** (critical for top-level constants accessed from functions). **Two-pass translation**:
  - **Pass 1 — mutation scan**. For each top-level `Assign(targets=[Name(name)], value=literal)` (where `literal` is `Constant`/`List`/`Tuple`/`Dict` of recursively-literal values): walk the rest of `Module.body` via `Pylixir.AST.walk_scope/3` (boundary stops at nested function/lambda/class/comprehension scopes — those have their own `name` bindings). If **any** of these mutate `name`, the Assign is **NOT** a module attribute candidate:
    - `Assign(targets=[Name(name), ...], ...)` — direct reassign.
    - `AugAssign(target=Name(name), ...)` — augmented reassign.
    - `AugAssign(target=Subscript(value=Name(name), ...), ...)` — T14 rewrites to reassign.
    - Statement-context method call `name.method(...)` where method ∈ T30's mutation table (`append`, `sort`, `update`, `add`, `discard`, `clear`, `pop`, `remove`, `extend`, `insert`, `reverse`) — T30 rewrites to reassign.
    - For-loop with `target=Name(name)` — rebinds.
  - **Pass 2 — partition + codegen**. With the mutation-free set known, emit:
    1. `function_defs`: `FunctionDef` nodes → module-level `defp`s (T19).
    2. `module_attrs`: literal-Assigns from Pass 1's mutation-free set → `@var_<name> <value>` module attributes (Elixir compile-time constants).
    3. `runtime_statements`: everything else in `Module.body` (including literal-Assigns that did get mutated later) → inside `def py_main`'s body, in original order.
...
- Without the mutation scan, the common Python pattern `DATA = [1, 2, 3]; DATA.append(4)` would raise "cannot reassign module attribute" — friction for code that clearly intends a mutable top-level list.
...
- Reassignment of a module-attr name would be impossible by construction (Pass 1 would have demoted it). Defensive raise still warranted for safety: if `Assign`/`AugAssign` targets a name present in `Context.module_attrs` → raise `UnsupportedNodeError` with hint "internal partition bug: module-attribute `<name>` should not be reassignable; report this".
...
- **`FunctionDef` position check via `Context.def_position`** (added in T03 alongside other Context fields):
  - `:module_top` — set only when converting `Module.body`'s direct `FunctionDef` children. Emit `defp`.
  - `:nested_fn` — set when entering a `FunctionDef`/`Lambda` body. Inner `FunctionDef` is T21's territory (emit as `fn`).
  - `:other` — set when entering any other body (`If`, `For`, `While`, `Try` arm, comprehension). T19 entered from `:other` → raise `UnsupportedNodeError` with hint "function definitions inside control-flow are not supported; lift to module level or use a `lambda`". Otherwise we'd emit `defp` inside an `if do ... end`, which is invalid Elixir.
...
- Centralised `Call` handler with **strict precedence**:
  ...
  2. Otherwise, branch by `Call.func` shape:
     - **`Name(id)` known cases**: math builtin → regular builtin → **explicit-unsupported-builtin** → known module-level function. None matched and `id` isn't locally bound → emit as-is `<id>(args)` (matches Python's NameError behavior at runtime).
     - **Explicit-unsupported-builtins list** lives in `Pylixir.Builtins.Registry.unsupported/0`. When `id` is in this list, raise `UnsupportedNodeError` with a per-builtin hint **at translation time** — much better UX than letting `globals(...)` emit as-is and surface later as an undefined-function warning (failing T04b's `diagnostics == []`) plus a cryptic runtime `UndefinedFunctionError`. List includes:
       - Introspection: `globals`, `locals`, `vars`, `dir`, `id`, `hash`, `callable`, `hasattr`, `getattr`, `setattr`, `delattr`.
       - Eval/exec/compile: `eval`, `exec`, `compile`, `__import__`.
       - I/O: `open`.
       - OOP: `super`, `object` (as a class — `isinstance` form handled in T26).
       - Iteration protocol: `iter`, `next`.
       - Bytes: `bytes`, `bytearray`, `memoryview`.
       - Format/repr: `repr`, `format`.
       - Interactive: `breakpoint`, `help`, `copyright`, `credits`, `license`, `exit`, `quit`.
...
- **Module-body partitioning for top-level constants (T05/T07/T19)** — Critical: defp's at module level can't see py_main's locals, so the common Python pattern `PI = 3.14; def area(r): return PI * r * r` would otherwise fail to compile. Fix: **two-pass translation**. Pass 1 scans `Module.body` via walk_scope (boundary-aware) for mutations to each candidate literal-Assign name. Pass 2 partitions: `FunctionDef` → defp's; mutation-free literal-Assigns → `@var_<name>` module attributes; everything else (including mutated literal-Assigns) → inside `py_main`. T07's `Name` converter emits `@var_<name>` for tracked module attrs. The mutation scan handles the common `DATA = []; DATA.append(...)` pattern correctly (DATA stays as a runtime var). Function bodies referencing non-literal/mutated top-level vars raise with hint suggesting parameter-passing.
- **Explicit-unsupported-builtins (T28)** — `Pylixir.Builtins.Registry.unsupported/0` enumerates Python builtins that have no Pylixir equivalent (`globals`, `eval`, `repr`, `getattr`, `iter`, etc.). T28's router checks this set in the `Name(id)` precedence and raises at translation time with a per-builtin hint. Avoids `UndefinedFunctionError`-as-cryptic-output. User-defined-but-missing names still emit as-is (Python NameError UX preserved).
- **`FunctionDef` position guard (T19)** — `Context.def_position` is set to `:module_top` for direct children of `Module.body`, `:nested_fn` when entering FunctionDef/Lambda bodies, `:other` everywhere else (If/For/While arms, etc.). T19 entered from `:other` raises with hint "lift to module level or use a lambda". Without this, `if cond: def foo(): ...` would emit `defp foo` inside an `if do ... end` block — invalid Elixir.
