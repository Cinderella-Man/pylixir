...
| 81 | 🔴 | `isinstance(True, int)` | `is_integer(true)` → `false` | Python returns `True` (bool subclass of int). `is_integer(x) \|\| is_boolean(x)`. | §11.31 |
| 82 | 🔴 | List out-of-bounds | `Enum.at(list, 100)` → `nil` | Python raises `IndexError`. Use `Enum.fetch!/2` or document. | §11.32 |
| 83 | 🔴 | `list[i] = x` out-of-bounds | `List.replace_at(list, 100, x)` → unchanged list | Python raises `IndexError`. Silent no-op. Document as known limitation. | §11.33 |
| 84 | 🟡 | `min`/`max` 3+ args | `min(a, b, c)` not handled | Wrap in list: `Enum.min([a, b, c])` | §12.8 |
